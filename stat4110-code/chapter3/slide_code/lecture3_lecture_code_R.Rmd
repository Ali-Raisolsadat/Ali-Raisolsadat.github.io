---
title: "Chapter 3 Lecture Code"
author: "Ali Raisolsadat"
date: "2025-10-10"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

```{r}
library(tidyverse)
library(ggplot2)
library(pracma)
```

### Lecture 1
```{r, out.width="100%"}
# Algorithm 1 - Slide 9
n <- 1000
U <- runif(n, min = 0, max = 1)
Y <- sin(pi * U)
estimator <- pi * mean(Y)
print(estimator)
```

```{r, out.width="100%"}
# true value of the integral
true_value <- 2
n_values <- seq(1, 5001, by = 10)
estimates <- numeric(length(n_values))

# compute Monte Carlo estimator for each n
for (i in seq_along(n_values)) {
  n <- n_values[i]
  U <- runif(n, min = 0, max = 1)
  Y <- sin(pi * U)
  estimates[i] <- pi * mean(Y)
}

# plot
plot(n_values, estimates, type = "l", col = "black", lwd = 1, 
     xlab = "Sample Size (n)", ylab = "Estimator Value",
     main = "Convergence of Monte Carlo Estimator")
abline(h = true_value, col = "red", lty = 2, lwd = 2)
legend("bottomright", legend = c("Monte Carlo Estimates", "True Value = 2"),
       col = c("black", "red"), lty = c(1,2), lwd = c(1,2))
grid()
```

```{r, out.width="100%"}
# Algorithm 2 - Slide 10
n <- 1000
U <- runif(n, min = 0, max = 1)
V <- runif(n, min = 0, max = 1)
Y <- sqrt(U + V) * exp(U * V)
estimator <- mean(Y)
print(estimator)
```

```{r, out.width="100%"}
# approximate true value using a very large n
n_true <- 2e6
U_true <- runif(n_true)
V_true <- runif(n_true)
Y_true <- sqrt(U_true + V_true) * exp(U_true * V_true)
true_value <- mean(Y_true)

# range of sample sizes
n_values <- seq(1, 5001, by = 10)
estimates <- numeric(length(n_values))

# compute Monte Carlo estimator for each n
for (i in seq_along(n_values)) {
  n <- n_values[i]
  U <- runif(n)
  V <- runif(n)
  Y <- sqrt(U + V) * exp(U * V)
  estimates[i] <- mean(Y)
}

# plot
plot(n_values, estimates, type = "l", col = "black", lwd = 1,
     xlab = "Sample Size (n)", ylab = "Estimator Value",
     main = "Convergence of Monte Carlo Estimator")
abline(h = true_value, col = "red", lty = 2, lwd = 2)
legend("bottomright", 
       legend = c("Monte Carlo Estimates", paste0("True Value ≈ ", round(true_value, 4))),
       col = c("black", "red"), lty = c(1,2), lwd = c(1,2))
grid()
```

### Lecture 3
```{r, out.width="100%"}
n <- 1000
m <- n/2
true_value <- exp(1) - 1

# antithetic variates
ant_u <- runif(m)
ant_g <- (exp(ant_u) + exp(1 - ant_u)) / 2
ant_estimator <- mean(ant_g)
ant_ci <- c(
  ant_estimator - 1.96 * sd(ant_g) / sqrt(m),
  ant_estimator + 1.96 * sd(ant_g) / sqrt(m)
)

# monte carlo
mc_u <- runif(n)
mc_g <- exp(mc_u)
mc_estimator <- mean(mc_g)
mc_ci <- c(
  mc_estimator - 1.96 * sd(mc_g) / sqrt(n),
  mc_estimator + 1.96 * sd(mc_g) / sqrt(n)
)

df <- data.frame(
  Method = c("Monte Carlo", "Antithetic"),
  Estimate = c(mc_estimator, ant_estimator),
  Lower = c(mc_ci[1], ant_ci[1]),
  Upper = c(mc_ci[2], ant_ci[2])
)

# plot
ggplot(df, aes(x = Method, y = Estimate)) +
  geom_point(size = 3, color = "blue") +
  geom_errorbar(aes(ymin = Lower, ymax = Upper), width = 0.1, color = "blue") +
  geom_hline(yintercept = true_value, linetype = "dashed", color = "red") +
  labs(title = "Monte Carlo vs Antithetic Variates Estimation of E(e^U)", 
       y = "Estimated Value") +
  theme_bw(base_size = 14)
```

### Part 4
```{r, out.width="100%"}
# basic Monte Carlo estimator
n <- 1000
U <-  runif(n)
G <-  exp(-U^4)
mc_estimator <- mean(G)
mc_ci <- c(
  mc_estimator - 1.96 * sd(G) / sqrt(n),
  mc_estimator + 1.96 * sd(G) / sqrt(n)
)

# importance sampling estimator
C <-  exp(1) / (exp(1) - 1)
U <-  runif(n)
Y <-  log(C / (C - U))
G1 <- 1/C * exp(Y - Y^4)
is_estimator <- mean(G1)
is_ci <- c(
  is_estimator - 1.96 * sd(G1) / sqrt(n),
  is_estimator + 1.96 * sd(G1) / sqrt(n)
)

# results
cat("Basic Monte Carlo Estimate:", mc_estimator, "\n95% CI:", mc_ci, "\n\n")
cat("Importance Sampling Estimate:", is_estimator, "\n95% CI:", is_ci, "\n")
```

### Part 5
```{r, out.width="100%"}
# basic Monte Carlo estimator
n <- 1000
U <- runif(n)
G <- log(2*U^2+1)
mc_estimator <- mean(G)
mc_ci <- c(
  mc_estimator - 1.96 * sd(G) / sqrt(n),
  mc_estimator + 1.96 * sd(G) / sqrt(n)
)

G1 <- log(2*U^2+1) - U
cv_estimator <- mean(G1) + 0.5
cv_ci <- c(
  cv_estimator - 1.96 * sd(G1) / sqrt(n),
  cv_estimator + 1.96 * sd(G1) / sqrt(n)
)

# results
cat("Basic Monte Carlo Estimate:", mc_estimator, "\n95% CI:", mc_ci, "\n\n")
cat("Control Variates Estimate:", cv_estimator, "\n95% CI:", cv_ci, "\n")

```

### Part 6
```{r, out.width="100%"}
n <- 1000
m <- 10
J <- 100
bias <- numeric(J)
theta <- numeric(J)

for (j in 1:J) {
  rational_p <- 3 + 2 * j / J
  theta[j] <- rational_p
  e <- numeric(n)
  
  for (i in 1:n) {
    u <- runif(m, 0, rational_p)
    e[i] <- max(u)
  }
  
  bias[j] <- mean(e) - rational_p
}

# plot
plot(theta, bias, col = "black", type='p', pch=19,
     xlab = expression(theta), ylab = "Estimated Bias")
title(main = expression("Monte Carlo Bias: " ~ hat(theta)))
grid()
```

```{r, out.width="100%"}
# OR
n <- 1000
m <- 10
J <- 100
theta <- 3 + 2 * (0:(J - 1)) / 100
bias <- numeric(J)

for (j in 1:J) {
  t <- theta[j]
  U <- matrix(runif(n * m, 0, t), nrow = n, ncol = m)  # n x m matrix
  est <- apply(U, 1, max)                              # row-wise max
  bias[j] <- mean(est) - t
}

# plot
plot(theta, bias, col = "black", type='p', pch=19,
     xlab = expression(theta), ylab = "Estimated Bias")
title(main = expression("Monte Carlo Bias: " ~ hat(theta)))
grid()
```
### Part 7
```{r, out.width="100%"}
theta <- c()
sd1 <- c()
sd2 <- c()
n <- 5000
m <- 20
J = 100

for (j in 1:(J+1)) {
  
  theta[j] <- 10 + (j - 1) / 10
  estim1 <- c()
  estim2 <- c()
  
  for (i in 1:n) {
    U <- runif(m, min = 0, max = theta[j])
    estim1[i] <- 21 * max(U) / 20
    estim2[i] <- 2 * mean(U)
  }
  
  sd1[j] <- sd(estim1)
  sd2[j] <- sd(estim2)
}

# Plots
par(mfrow = c(1, 2))

plot(theta, sd1, type = "p", col = "black", pch = 19,
     xlab = expression(theta), ylab = "SD of Estimator 1",
     main = expression("Monte Carlo SD:" ~ hat(theta)[1]))
grid()

plot(theta, sd2, type = "p", col = "black", pch = 19,
     xlab = expression(theta), ylab = "SD of Estimator 2",
     main = expression("Monte Carlo SD:" ~ hat(theta)[2]))
grid()

```

### Part 8
```{r, out.width="100%"}
n <- 10
N <- 5000
lambda <- numeric(19)
P <- numeric(19)

for (i in 1:19) {
  lambda[i] <- 0.05 * (i + 1)
  k <- 0
  for (j in 1:N) {
    X <- rpois(n, lambda[i])
    m <- mean(X)
    d <- sd(X) * qt(0.975, df = n - 1) / sqrt(n)
    if (m - d <= lambda[i] && lambda[i] <= m + d) {
      k <- k + 1
    }
  }
  P[i] <- k / N
}

# plot
plot(lambda, P, 
     type = "b", pch = 19, lwd = 2, col = "black",
     xlab = expression(lambda), ylab = "Coverage Probability (P)",
     main = expression("Confidence Coefficients for " * lambda),
     cex.main = 1.2, cex.lab = 1.1, cex.axis = 0.9)
grid()

```
