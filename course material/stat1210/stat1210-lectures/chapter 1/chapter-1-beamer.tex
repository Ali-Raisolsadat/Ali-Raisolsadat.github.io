...
\begin{frame}{The Course Roadmap: The 5-Part Journey}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \textbf{I: Exploring Data (The Basics)}
      \begin{itemize}
...
\begin{frame}{What Is Statistics?}
  ...
  \begin{itemize}
    ...
    \item summarize patterns;
    \item compare groups; and
    \item use data to answer questions.
  \end{itemize}
  \vspace{0.15cm}
...
\begin{frame}{Statistics Is Everywhere}
  \begin{itemize}
    ...
    \item \textbf{Economics:} Are grocery prices increasing over time, or did one unusual month distort our impression?
  ...
  \end{itemize}
  \vspace{0.2cm}
  \begin{block}{Common First Step}
    Before making a conclusion, \textbf{look carefully at the data}.
  \end{block}
\end{frame}

% SLIDE 6
\begin{frame}{Why Statistics Still Matters in the Age of AI}
  Modern AI can process huge datasets and create tables or graphs almost instantly.
  \vspace{0.15cm}
  \begin{columns}[T]
    \begin{column}{0.47\textwidth}
      \begin{block}{AI Can Help Us}
        \begin{itemize}
  ...
  \vspace{0.15cm}
  \centering
  \textbf{Fast analysis is not automatically good analysis.}
\end{frame}
...
\begin{frame}{The Coding Trap: Numbers Can Still Be Categories}
  ...
  \[
    1 = \text{Control}, \qquad 2 = \text{Fertilizer A}, \qquad 3 = \text{Fertilizer B}.
  \]
  \begin{itemize}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{Discrete: Usually Counting}
        ...
        \end{itemize}
        A plant can have 7 or 8 leaves, but not 7.42 leaves.
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{Continuous: Usually Measuring}
        \begin{itemize}
          ...
          \item Concentration
        \end{itemize}
        More precise instruments can record finer values.
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
...
% SLIDE 14
\begin{frame}{Count or Measure? A Simple First-Year Rule}
  \begin{table}
    \centering
    \begin{tabular}{lll}
      \toprule
...
\begin{frame}{Bar Graphs: The Default Choice for Categorical Data}
  ...
  \end{itemize}
  \vspace{0.15cm}
  \begin{alertblock}{When Reading a Bar Graph, Ask}
...
\begin{frame}{Anatomy of a Bar Graph}
  \centering
  \lectureimage[0.92\linewidth]{fig_bar_anatomy.png}
\end{frame}
...
\begin{frame}{Pie Charts: Parts of One Whole}
  ...
  \begin{enumerate}
    ...
    \item categories represent the \textbf{entire whole}; and
...
% SLIDE 23
\begin{frame}{Choosing the Wrong Graph: What Can Go Wrong?}
  ...
  \begin{alertblock}{Statistical Literacy}
    Creating a graph is only half the job. You must also be able to judge whether a graph is \textbf{appropriate and fair}.
  \end{alertblock}
\end{frame}

% SLIDE 24
\begin{frame}{Mistake 1: Using a Line Graph for Nominal Categories}
  \begin{columns}[c]
    \begin{column}{0.58\textwidth}
      \centering
...
% SLIDE 25
\begin{frame}{Mistake 2: Truncating the Axis of a Bar Graph}
  \begin{columns}[c]
    \begin{column}{0.58\textwidth}
      \centering
...
% SLIDE 26
\begin{frame}{Mistake 3: A Pie Chart with Different Denominators}
  \begin{columns}[c]
    \begin{column}{0.58\textwidth}
      \centering
...
\begin{frame}{How a Histogram Works}
  ...
  \begin{alertblock}{Crucial Difference from a Bar Graph}
    Histogram bars \textbf{touch} because the horizontal axis is a continuous numerical scale.
  \end{alertblock}
\end{frame}
...
\begin{frame}{How to Describe a Quantitative Distribution: SOCS}
  ...
  \end{description}
  \vspace{0.2cm}
  \begin{alertblock}{Always Use Context}
    Mention the \textbf{variable name and units}, not only ``right-skewed'' or ``one outlier.''
  \end{alertblock}
\end{frame}
...
\begin{frame}{Dotplots: Showing Individual Values}
  ...
  \end{itemize}
  \begin{block}{Use a Dotplot When}
...
\begin{frame}{Time Plots: When Order Matters}
  ...
  \vspace{0.15cm}
  When chronological order matters, use a \textbf{time plot}:
  \begin{itemize}
