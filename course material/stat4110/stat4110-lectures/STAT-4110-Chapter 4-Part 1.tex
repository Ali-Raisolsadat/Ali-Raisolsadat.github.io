...
%--------------------------------------------
% Slide 4: Markov Chains for Monte Carlo Estimation
%--------------------------------------------
...
%--------------------------------------------
\begin{frame}{Convergence to the Actual Expectation - Pointless MCMC in Action}
\begin{center}
\includegraphics[width=\textwidth]{chapter4-part1-plot0.png}
\end{center}
\end{frame}
...
\begin{enumerate}
  ...
  \item Until we get bored:
    \begin{enumerate}
      ...
      \[
        u \le \frac{\tilde{p}(\hat{x}^{(t)})}{\tilde{p}(x^{(t-1)})} , \quad u \le \frac{\text{probability of proposed}}{\text{probability of current}}
      \]
      and set $x^{(t)} = \hat{x}^{(t)}$.
...
%--------------------------------------------
% Slide X: 1D Graphical Intuition for Metropolis
%--------------------------------------------
\begin{frame}{1D Graphical Intuition: Why Some Proposals Are Accepted}
...
\begin{itemize}
  \item Suppose the chain is currently at $x^{(t-1)}$.
  \item A proposal $\hat{x}$ is generated by adding Gaussian noise:
  \[
...
\begin{block}{Two possible situations}
\begin{itemize}
  \item \textbf{Uphill move:} $p(\hat{x}) \ge p(x^{(t-1)})$  
  \[
    \frac{\tilde p(\hat{x})}{\tilde p(x^{(t-1)})} \ge 1
  \]
  The proposal is accepted with probability $1$.
  
  \item \textbf{Downhill move:} $p(\hat{x}) < p(x^{(t-1)})$  
  \[
...
\begin{itemize}
  \item The chain always climbs toward higher density.
  \item It sometimes moves downhill to avoid getting stuck.
\end{itemize}
