...
%-------------------
\begin{frame}{Conditional Distributions Example}
\begin{center}
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \While{$\text{counter} \leq n$}
            \State Generate $U_1 \sim \text{Uniform}(-1,1)$
            \State Generate $U_2 \sim \text{Uniform}(0,1)$
            \If{$U_1^2 + U_2^2 < 1$}
                \State $X_1[\text{counter}] \gets U_1$
                \State $X_2[\text{counter}] \gets U_2$
                \State $\text{counter} \gets \text{counter} + 1$
            \EndIf
        \EndWhile
        \State \textbf{Output:} $X_1, X_2$ (accepted points inside the semicircle)
...
\begin{enumerate}
	...
	\item Implement Algorithms 2 and 3 using the rectangle $[-1,1] \times [0,1]$. If computationally feasible, run both algorithms for $num\_sims = [50, 100, 500, 1000, 2000]$ and plot the difference in their running times.
		\begin{itemize}
	          ...
	          \end{itemize}
	      \end{itemize}
	\item Write code to generate a random sample of size 1000 from the conditional distribution of $X \sim Binomial(n,p)$  (with $n=10$ and probability of success $p=0.6$) conditioned on $X\geq 5$. 
	\item Write code to generate (and plot) a sample of 500 uniformly distributed random points on the set of plane given by $y\geq 0$, $-\pi/2 \leq  x \leq \pi/2$, and $y \leq \cos x$.
\end{enumerate}
\end{frame}
