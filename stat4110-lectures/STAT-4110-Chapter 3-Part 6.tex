...
There are several cases in which we can find the bias and variance explicitly, for example
...
In other cases, finding the bias and the variance of an estimator explicitly is not possible. It is in situations like these that Monte Carlo methods may be useful.
\end{frame}
...
\begin{algorithmic}[1]
  ...
  \For{$j = 1$ to $J$}
    ...
    \State Store $\texttt{theta}[j] = \theta_j$
    ...
    \EndFor
    \State Compute $\texttt{mean\_max}_j = \dfrac{1}{n} \sum_{i=1}^n \texttt{samples}[i]$
    \State Compute $\texttt{bias}[j] = \texttt{mean\_max}_j - \theta_j$
  \EndFor
...
\begin{frame}[fragile]
\begin{algorithm}[H]
\caption{Vectorized Monte Carlo Bias Estimation for $\hat{\theta} = \max(U_1, \ldots, U_m)$}
\begin{algorithmic}[1]
  \State \textbf{Input:} Number of replications $n = 1000$, number of samples $m = 10$, number of parameter values $J = 100$
  \State Initialize arrays: $\texttt{bias}[1:J]$, $\texttt{theta}[1:J]$
  \For{$j = 1$ to $J$}
    \State Set $\theta_j = 3 + 2\dfrac{j}{J}$
    \State Store $\texttt{theta}[j] = \theta_j$
    \State Generate matrix $U \sim \text{Uniform}(0,\theta_j)$ of size $(n,m)$
    \State Compute vector $\hat{\boldsymbol{\theta}} = (\max_{k} U_{1k},\ldots,\max_{k} U_{nk})$
    \State Compute $\texttt{bias}[j] = \dfrac{1}{n}\sum_{i=1}^{n} \hat{\theta}_i - \theta_j$
  \EndFor
  \State \textbf{Output:} Arrays $\texttt{bias}[1:J]$ and $\texttt{theta}[1:J]$
\end{algorithmic}
\end{algorithm}
