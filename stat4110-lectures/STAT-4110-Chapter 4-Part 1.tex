...
\textbf{Definition:}  
A sequence of random variables $\{X_t\}_{t \ge 0}$ is a \textbf{Markov chain} if
\[
...
\medskip
\textbf{n-step transition:}  
$P^{(n)} = \mathbf{P}^n$, with entries $P_{ij}^{(n)} = P(X_{t+n} = j \mid X_t = i)$.
...
\textbf{Definition:}  
A probability vector $\boldsymbol{\pi}$ is a \textbf{stationary distribution} of a Markov chain if
\[
...
If $X_0 \sim \boldsymbol{\pi}$, then $X_t \sim \boldsymbol{\pi}$ for all $t$.  
The chain remains in equilibrium under $\boldsymbol{\pi}$.
...
\medskip
This limiting behavior holds under certain conditions
...
\medskip
\textbf{Recurrence:}  
State $i$ is \textbf{recurrent} if, starting from $i$, the probability of returning to $i$ is 1:
\[
...
\medskip
\textbf{Ergodicity:}  
An irreducible, aperiodic, and positive recurrent Markov chain is called \textbf{ergodic}.  
For ergodic chains,
\[
\lim_{t \to \infty} P(X_t = j \mid X_0 = i) = \pi_j
\]
...
Consider estimating the expected value of a fair six-sided die.  
...
\textbf{Metropolis algorithm:} sampling from a \alert{continuous target distribution} $p(x)$.  
We assume $p(x)$ can be evaluated up to a normalizing constant:
\[
